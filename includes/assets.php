<?php
	...
	class OPanda_AssetsManager {
		...
		/**
		 * Cache for the method isPageSelected.
		 *
		 * @since 3.0.0
		 * @var mixed
		 */
		private static $_cache_isPageSelected = array();
		/**
		...
		public static function autoUnlock($itemId)
		{
			...
			if( isset(self::$_autoUnlock[$itemId]) ) {
		...
		public static function isAutoUnlock($itemId)
		{
			...
			$filterResult = apply_filters('bizpanda_auto_unlock', null, $itemId);
			
			//todo: Фильтр opanda_auto_unlock устарел
			if( empty($filterResult) ) {
				$filterResult = factory_000_apply_filters_deprecated("opanda_auto_unlock", array(
					null,
					$itemId
				), '1.2.4', "bizpanda_auto_unlock");
			}
		...
		public static function handlePasscode()
		{
			...
			if( $permanentPasscode ) {
				...
				if( isset($_GET[$passcode]) ) {
					...
					if( empty(self::$_cookiesPassCode) ) {
						self::$_cookiesPassCode = 'opanda_' . wp_create_nonce('passcode');
					}
					if( isset($_COOKIE[self::$_cookiesPassCode]) || self::$_passcodeCookieSet ) {
						return true;
					}
		...
		public static function requestAssets($itemId, $fromBody = false, $fromHeader = false)
		{
			...
			//todo: хук является устаревшим opanda_request_resources
			factory_000_do_action_deprecated("opanda_request_resources", array(), '1.2.4', "bizpanda_request_resources");
			
			do_action('bizpanda_request_resources');
			
			//todo: хук является устаревшим opanda_request_assets_for_
			factory_000_do_action_deprecated('opanda_request_assets_for_' . $type, array(
				$itemId,
				$options,
				$fromBody,
				$fromHeader
			), '1.2.4', "bizpanda_request_assets_for_" . $type);
			
			do_action('bizpanda_request_assets_for_' . $type, $itemId, $options, $fromBody, $fromHeader);
		}
		...
		public static function requestLockerAssets()
		{
			...
			if( isset(self::$_requested['locker-assets']) ) {
				return;
			}
		...
		public static function defineVisibilityVars($options, $itemId)
		{
			...
			if( empty($options['opanda_visibility_filters']) ) {
				return;
			}
			...
			foreach($params as $param) {
				$value = apply_filters('bizpanda_visibility_param_' . $param, null);
				
				//todo: Фильтр bp_visibility_param_ устарел
				if( empty($value) ) {
					$value = factory_000_apply_filters_deprecated('bp_visibility_param_' . $param, array(null), '1.2.4', 'bizpanda_visibility_param_' . $param);
				}
				
				//todo: Фильтр bp_visibility_param устарел
				$value = factory_000_apply_filters_deprecated('bp_visibility_param', array(
					$value,
					$param
				), '1.2.4', 'bizpanda_visibility_param');
				$value = apply_filters('bizpanda_visibility_param', $value, $param);
		...
		public static function connectLockerAssets()
		{
			...
			wp_enqueue_style('opanda-lockers', OPANDA_BIZPANDA_URL . '/assets/css/lockers.min.css', array(), $bizpanda->version);
			
			wp_enqueue_script('opanda-lockers', OPANDA_BIZPANDA_URL . '/assets/js/lockers.min.js', array(
				'jquery',
				'jquery-effects-core',
				'jquery-effects-highlight'
			), $bizpanda->version, true);
			
			wp_enqueue_script('opanda-lockers-autoloader', OPANDA_BIZPANDA_URL . '/assets/js/lockers.autoloader.js', array(
				'opanda-lockers'
			), $bizpanda->version, true);
			
			if( get_locale() == 'ru_RU' ) {
				wp_enqueue_script('opanda-opanda-lockers-localization', OPANDA_BIZPANDA_URL . '/assets/js/lockers.localization.ru_RU.min.js', array(
					'opanda-lockers'
				), $bizpanda->version, true);
			}
			...
			if( get_option('opanda_debug', false) ) {
				...
				wp_enqueue_style('opanda-lockers-debugger', OPANDA_BIZPANDA_URL . '/assets/css/lockers.debugger.css', array(), $bizpanda->version);
				
				if( get_locale() == 'ru_RU' ) {
					wp_enqueue_script('opanda-lockers-debugger', OPANDA_BIZPANDA_URL . '/assets/js/lockers.debugger.ru_RU.js', array('opanda-lockers'), $bizpanda->version, true);
				} else {
					wp_enqueue_script('opanda-lockers-debugger', OPANDA_BIZPANDA_URL . '/assets/js/lockers.debugger.js', array('opanda-lockers'), $bizpanda->version, true);
				}
			}
			...
			//todo: хук является устаревшим opanda_connect_locker_assets
			factory_000_do_action_deprecated("opanda_connect_locker_assets", array(), '1.2.4', "bizpanda_connect_locker_assets");
			
			do_action('bizpanda_connect_locker_assets');
		}
		...
		{
			require_once(OPANDA_BIZPANDA_DIR . '/includes/functions.php');
		...
		{
			//todo: хук является устаревшим opanda_before_locker_creator_script
			factory_000_do_action_deprecated("opanda_before_locker_creator_script", array(), '1.2.4', "bizpanda_before_locker_creator_script");
			
			do_action('bizpanda_before_locker_creator_script');
			...
			<script>
				...
				(function($) {
					$(function() {
						...
						$.getScript("<?php echo $robustLoader; ?>", function() {
							if( window.bizpanda && window.bizpanda.initLockers ) {
								window.bizpanda.initLockers();
							}
						});
			...
			//todo: хук является устаревшим opanda_after_locker_creator_script
			factory_000_do_action_deprecated("opanda_after_locker_creator_script", array(), '1.2.4', "bizpanda_after_locker_creator_script");
			
			do_action('bizpanda_after_locker_creator_script');
		}
		...
		public static function requestFacebookSDK()
		{
			...
			if( isset(self::$_requested['facebook-sdk']) ) {
				return;
			}
		...
		public static function connectFacebookSDK()
		{
			...
			$fb_version = get_option('opanda_facebook_version', 'v2.8');
			$fb_lang = get_option('opanda_lang', 'en_EN');
			...
			<!-- / -->
			<?php
			//todo: хук является устаревшим opanda_connect_facebook_sdk
			factory_000_do_action_deprecated("opanda_connect_facebook_sdk", array(), '1.2.4', "bizpanda_connect_facebook_sdk");
			
			do_action('bizpanda_connect_facebook_sdk');
		}
		...
		public static function printCssSelectorOptions()
		{
			...
			<style>
				...
				<?php echo (trim($options['css-selector']) == 'body')? 'onp-sl-body-inner-wrap' : $options['css-selector'] ?>
				{
					display: none
				;
				}
		...
		public static function printLockerOptions()
		{
			...
			<?php
			//todo: хук является устаревшим opanda_print_batch_locker_assets
			factory_000_do_action_deprecated("opanda_print_batch_locker_assets", array(
				$id,
				$item['options'],
				$item['name']
			), '1.2.4', "bizpanda_print_bulk_locker_assets");
			
			do_action('bizpanda_print_bulk_locker_assets', $id, $item['options'], $item['name']);
			?>
		<?php } ?>
			<!-- / -->
			<?php
		...
		public static function getBaseOptions($id)
		{
			...
			if( onp_build('free') ) {
				...
				$showCloseButtonDelay = intval(self::getLockerOption($id, 'close_button_show_delay', 0));
				$showCloseButtonDelay = is_numeric($showCloseButtonDelay)
					? $showCloseButtonDelay * 1000
					: 0;
				
				$showDelay = intval(self::getLockerOption($id, 'locker_show_delay', 0));
				$showDelay = is_numeric($showDelay)
					? $showDelay * 1000
					: 0;
				
				$params['locker']['close'] = self::getLockerOption($id, 'close');
				$params['locker']['showCloseButtonDelay'] = $showCloseButtonDelay;
				
				$params['locker']['showDelay'] = $showDelay;
				...
				if( 'advanced' === $visibilityMode ) {
					...
					if( !empty($visabilityConditions) ) {
						$visabilityConditions = json_decode($visabilityConditions);
					}
		...
		public static function getLockerDataToPrint($id, $lockData = array())
		{
			...
			//todo: Фильтр opanda_{$itemType}_item_options устарел
			$options = factory_000_apply_filters_deprecated("opanda_{$itemType}_item_options", array(
				$baseOptions,
				$id
			), '1.2.4', "bizpanda_{$itemType}_item_options");
			$options = apply_filters("bizpanda_{$itemType}_item_options", $options, $id);
			
			//todo: Фильтр opanda_item_options устарел
			$options = factory_000_apply_filters_deprecated("opanda_item_options", array(
				$options,
				$id
			), '1.2.4', "bizpanda_item_options");
			$options = apply_filters("bizpanda_item_options", $options, $id);
		...
		public static function getLockerOptions($lockerId)
		{
			...
			if( isset(self::$_lockerOptions[$lockerId]) ) {
				return self::$_lockerOptions[$lockerId];
			}
		...
		public static function normilizerMarkup($contentBefore, $contentInside, $shortcodeStart, $shortcodeEnd)
		{
			...
			if( !$normalizeMarkup ) {
				return $contentBefore . $shortcodeStart . $contentInside . $shortcodeEnd;
			}
			
			list($endingElements, $endingTags) = self::findMarkupElements(true, $contentInside);
			
			$allowedNames = array();
			foreach($endingElements as $element)
				$allowedNames[] = $element['name'];
		...
		public static function initBulkLocking()
		{
			...
			foreach($bulkLockers as $id => $options) {
				...
				if( !OPanda_Items::isAvailable($itemType) ) {
					continue;
				}
				...
				if( $options['way'] == 'css-selector' || $options['way'] == 'fullscreen-lock' ) {
					
					$lockData = self::getLockerDataToPrint($id);
					
					if( $options['way'] == 'fullscreen-lock' && self::isPageSelected($id, $options) && !self::isPageExcluded($id, $options) ) {
					...
					if( self::isPageSelected($id, $options) && !self::isPageExcluded($id, $options) ) {
						self::requestAssets($id);
						continue;
					}
				}
		...
		// todo: решить проблемы с вложенными массивами
		public static function getResToPrint($requestedTextRes)
		{
			$resToPrint = array();
			foreach($requestedTextRes as $key => $res) {
				/*if( is_array($res) ) {
					$requestedTextRes[$key] = self::getResToPrint($res);
					return $requestedTextRes;
				}*/
				$value = get_option('opanda_res_' . $res, false);
				if( false === $value ) {
					continue;
		 ...
		 * @return boolean
		 */
		public static function isPageSelected($id, $options)
		{
			global $post;
			if( empty($post) ) {
				return true;
			...
			$key = $id . '' . $post->ID;
			if( isset(self::$_cache_isPageSelected[$key]) ) {
				return self::$_cache_isPageSelected[$key];
			}
			
			if( empty($options['select_posts']) ) {
				self::$_cache_isPageSelected[$key] = true;
				
				return true;
			}
			
			if( !in_array($post->ID, $options['select_posts']) ) {
				self::$_cache_isPageSelected[$key] = false;
		 ...
		 * @return boolean
		 */
		public static function isPageExcluded($id, $options)
		{
			global $post;
			if( empty($post) ) {
				return true;
			...
			$key = $id . '' . $post->ID;
			if( isset(self::$_cache_isPageExcluded[$key]) ) {
				return self::$_cache_isPageExcluded[$key];
			}
		...
		public static function addSocialLockerShortcodes($content)
		{
			...
			foreach($bulkLockers as $id => $options) {
				...
				if( !self::isPageSelected($id, $options) ) {
					continue;
				}
		...
		{
			$bulkLockers = get_option('onp_sl_bulk_lockers', array());
			if( isset($bulkLockers[$id]) ) {
				unset($bulkLockers[$id]);
			}
		...
		public static function printDynamicThemesOptions()
		{
			...
			<?php
			//todo: хук является устаревшим opanda_print_dynamic_theme_options
			factory_000_do_action_deprecated("opanda_print_dynamic_theme_options", array(), '1.2.4', "bizpanda_print_dynamic_theme_options");
			
			do_action('bizpanda_print_dynamic_theme_options');
			?>
			<!-- / -->
		<?php
		}
	...
	if( !is_admin() ) {
		add_action('template_redirect', 'OPanda_AssetsManager::init');
	}
