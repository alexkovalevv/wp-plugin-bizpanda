<?php
	 ...
	 * @since 1.0.0
	 */
	class OPanda_BulkLockingMetaBox extends FactoryMetaboxes000_Metabox {
		...
		{
			$scripts->add(OPANDA_BIZPANDA_URL . '/assets/admin/js/libs/select2/select2.min.js');
			$styles->add(OPANDA_BIZPANDA_URL . '/assets/admin/css/libs/select2.css');

			$scripts->add(OPANDA_BIZPANDA_URL . '/assets/admin/js/metaboxes/bulk-lock.010002.js');
		}
		...
		public function html()
		{
			...
			$setupStateClass = empty($options)
				? 'onp-sl-empty-state'
				: 'onp-sl-has-options-state';

			// Классы к информатифным блокам в редакторе замков
			$wayStateClass = '';
			...
			//todo: Фильтр onp_bp_bulk_locking_way_state_class устарел
			$wayStateClass = factory_000_apply_filters_deprecated('onp_bp_bulk_locking_way_state_class', array(
				$wayStateClass,
				$options
			), '1.2.4', 'bizpanda_bulk_locking_way_state_class');

			$wayStateClass = apply_filters('bizpanda_bulk_locking_way_state_class', $wayStateClass, $options);
			...
			$interrelatedClass = (!$interrelated)
				? 'onp-sl-not-interrelation'
				: '';
			...
			<script>
				...
				if( !window.bizpanda.lang ) {
					window.bizpanda.lang = {};
				}
				...
				window.bizpanda.lang.selectPosts =
					"<?php _e('Only posts: %s', 'bizpanda') ?>";
			...
			<div class="factory-bootstrap-000 factory-fontawesome-000">
				...
				<div class="onp-sl-setup-section <?php echo $setupStateClass ?>">
					...
					<div class="onp-sl-has-options-content <?php echo $wayStateClass ?> <?php echo $ruleStateClass ?>">
						...
						<?php do_action('bizpanda_bulk_locking_add_way_description'); ?>

						<div class='onp-sl-rules'>
                        <span class='onp-sl-post-types-rule'>
                            <?php printf(__('Applies to types: %s', 'bizpanda'), $postTypes) ?>
                        </span>
						<span class='onp-sl-select-post-ids-rule'>
                            <?php printf(__('Only selected pages: %s', 'bizpanda'), implode(',', $selectPosts)) ?>
                        </span>
                        <span class='onp-sl-exclude-post-ids-rule'>
                            <?php printf(__('Excludes posts: %s', 'bizpanda'), implode(',', $excludePosts)) ?>
                        </span>
                        <span class='onp-sl-exclude-categories-ids-rule'>
                            <?php printf(__('Excludes categories: %s', 'bizpanda'), implode(',', $excludeCategories)) ?>
                        </span>
						</div>
						...
						<div class="modal-content">
							...
							<div class="modal-body">
								<div class="btn-group" id="onp-sl-bulk-locking-way-selector" data-toggle="buttons-radio">
									<?php
										//todo: хук является устаревшим onp_bp_bulk_locking_add_tab_before
										factory_000_do_action_deprecated("onp_bp_bulk_locking_add_tab_before", array($defaultWay), '1.2.4', "bizpanda_bulk_locking_add_tab_before");

										do_action('bizpanda_bulk_locking_add_tab_before', $defaultWay);
									?>
									<button data-target="#onp-sl-skip-lock-options" type="button" class="btn btn-default value skip-lock <?php if( $defaultWay == 'skip-lock' ) {
										echo 'active';
									} ?>" data-name="skip-lock">
										<i class="fa fa-sort-amount-desc"></i> <?php _e('Skip & Lock', 'bizpanda') ?>
									</button>
									<button data-target="#onp-sl-more-tags-options" type="button" class="btn btn-default value more-tag <?php if( $defaultWay == 'more-tag' ) {
										echo 'active';
									} ?>" data-name="more-tag">
										<i class="fa fa-scissors"></i> <?php _e('More Tag', 'bizpanda') ?></button>
									<button data-target="#onp-sl-css-selector-options" type="button" class="btn btn-default value css-selector <?php if( $defaultWay == 'css-selector' ) {
										echo 'active';
									} ?>" data-name="css-selector">
										<i class="fa fa-hand-o-up"></i> <?php _e('CSS Selector', 'bizpanda') ?></button>
									<?php
										//todo: хук является устаревшим onp_bp_bulk_locking_add_tab_after
										factory_000_do_action_deprecated("onp_bp_bulk_locking_add_tab_after", array($defaultWay), '1.2.4', "bizpanda_bulk_locking_add_tab_after");

										do_action('bizpanda_bulk_locking_add_tab_after', $defaultWay);
									?>
								</div>

								<?php
									//todo: хук является устаревшим onp_bp_bulk_locking_add_tab_content
									factory_000_do_action_deprecated("onp_bp_bulk_locking_add_tab_content", array(
										$options,
										$defaultWay,
										$types
									), '1.2.4', "bizpanda_bulk_locking_add_tab_content");

									do_action('bizpanda_bulk_locking_add_tab_content', $options, $defaultWay, $types);
								?>

								<div id="onp-sl-skip-lock-options" class="onp-sl-bulk-locking-options <?php if( $defaultWay !== 'skip-lock' ) {
									echo 'hide';
								} ?>">
								...
								<div id="onp-sl-more-tags-options" class="onp-sl-bulk-locking-options <?php if( $defaultWay !== 'more-tag' ) {
									echo 'hide';
								} ?>">
									<div class="onp-sl-description">
													...
													<?php foreach($types as $type) { ?>
														<li>
															<label for='onp-sl-post-type-<?php echo $type->name ?>-more-tag'>
																<input type='checkbox' class='onp-sl-post-type onp-sl-<?php echo $type->name ?>' id='onp-sl-post-type-<?php echo $type->name ?>-more-tag' value='<?php echo $type->name ?>' <?php if( in_array($type->name, $checkedPostTypes) ) {
																	echo 'checked="checked"';
																} ?> />
																<?php echo $type->label ?>
															</label>
								...
								<div id="onp-sl-css-selector-options" class="onp-sl-bulk-locking-options <?php if( $defaultWay !== 'css-selector' ) {
									echo 'hide';
								} ?>">
									<div class="onp-sl-description">
										<p>
		...
		public function save($postId)
		{
			...
			if( !is_array($bulkLockers) ) {
				$bulkLockers = array();
			}
			if( isset($bulkLockers[$postId]) ) {
				unset($bulkLockers[$postId]);
			}
			...
			if( !isset($_POST['onp_sl_way']) ) {
				...
				$allowClearWays = array('skip-lock', 'more-tag', 'css-selector');

				//todo: Фильтр onp_bp_bulk_locking_allow_clear_ways устарел
				$allowClearWays = factory_000_apply_filters_deprecated('onp_bp_bulk_locking_allow_clear_ways', array($allowClearWays), '1.2.4', 'bizpanda_bulk_locking_allow_clear_ways');
				$allowClearWays = apply_filters('bizpanda_bulk_locking_allow_clear_ways', $allowClearWays);

				if( !in_array($way, $allowClearWays) ) {
					delete_post_meta($postId, 'opanda_bulk_locking');
					delete_option('onp_sl_bulk_lockers');
					add_option('onp_sl_bulk_lockers', $bulkLockers);
				...
				$allowSaveWays = array('skip-lock', 'more-tag');

				//todo: Фильтр onp_bp_bulk_locking_allow_save_ways устарел
				$allowSaveWays = factory_000_apply_filters_deprecated('onp_bp_bulk_locking_allow_save_ways', array($allowSaveWays), '1.2.4', 'bizpanda_bulk_locking_allow_save_ways');
				$allowSaveWays = apply_filters('bizpanda_bulk_locking_allow_save_ways', $allowSaveWays);

				if( in_array($way, $allowSaveWays) ) {
					$postTypes = isset($_POST['onp_sl_post_types'])
						? $_POST['onp_sl_post_types']
						: '';

					$selectPosts = isset($_POST['onp_sl_select_posts'])
						? $_POST['onp_sl_select_posts']
						: '';

					$excludePosts = isset($_POST['onp_sl_exclude_posts'])
						? $_POST['onp_sl_exclude_posts']
						: '';
					$excludeCategories = isset($_POST['onp_sl_exclude_categories'])
						? $_POST['onp_sl_exclude_categories']
						: '';

					$data['post_types'] = explode(',', $postTypes);
					$data['post_types'] = !empty($data['post_types'])
						? array_map('trim', $data['post_types'])
						: array();

					$data['select_posts'] = !empty($selectPosts)
						? $this->_normalizeIntValArray(explode(',', $selectPosts))
						: array();
				...
				//todo: Фильтр onp_bp_bulk_locking_save_data устарел
				$data = factory_000_apply_filters_deprecated('onp_bp_bulk_locking_save_data', array($data), '1.2.4', 'bizpanda_bulk_locking_save_data');
				$data = apply_filters('bizpanda_bulk_locking_save_data', $data);
	...
	FactoryMetaboxes000::register('OPanda_BulkLockingMetaBox', $bizpanda);
/*@mix:place*/