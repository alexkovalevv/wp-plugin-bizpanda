<?php
	 ...
	 * @since 1.0.0
	 */
	class OPanda_PreviewMetaBox extends FactoryMetaboxes000_Metabox {
		...
		{
			global $bizpanda;
			$query_string = '?action=onp_sl_preview&from=post';

			//todo: Фильтр opanda_preview_url устарел
			$query_string = factory_000_apply_filters_deprecated('opanda_preview_url', array($query_string), '1.2.4', 'bizpanda_preview_url');
			$query_string = apply_filters('bizpanda_preview_url', $query_string);
			...
			//todo: Фильтр onp_sl_preview_data_wrap устарел
			$extra_data = factory_000_apply_filters_deprecated('onp_sl_preview_data_wrap', array($extra_data), '1.2.4', 'bizpanda_preview_data_wrap');
			$extra_data = apply_filters('bizpanda_preview_data_wrap', $extra_data);
			...
			<script>
				...
				window.opanda_vk_app_id = '<?php echo get_option('opanda_vk_appid') ?>';
			...
			<?php
			//todo: хук является устаревшим opanda_preview_metabox_print_scripts
			factory_000_do_action_deprecated("opanda_preview_metabox_print_scripts", array(), '1.2.4', "bizpanda_preview_metabox_print_scripts");

			do_action('bizpanda_preview_metabox_print_scripts');
			?>

			<p class="note"><strong><?php _e('Note', 'bizpanda'); ?>
					:</strong> <?php _e('In the preview mode, some features of the locker may not work properly.', 'bizpanda'); ?>
			</p>
	...
	global $bizpanda;
	FactoryMetaboxes000::register('OPanda_PreviewMetaBox', $bizpanda);
/*@mix:place*/