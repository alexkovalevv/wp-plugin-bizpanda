<?php
	 ...
	 * @since 1.0.0
	 */
	class OPanda_LockerShortcode extends FactoryShortcodes000_Shortcode {
		...
		public function html($attr, $content)
		{
			...
			if( onp_build('premium', 'ultimate', 'offline') ) {
			...
			//todo: Фильтр onp_sl_what_to_show устарел
			$whatToShow = factory_000_apply_filters_deprecated("onp_sl_what_to_show", array(
				$whatToShow,
				$id
			), '1.2.4', "bizpanda_shortcodes_what_to_show");

			$whatToShow = apply_filters('bizpanda_shortcodes_what_to_show', $whatToShow, $id);
			...
			$lockData['options']['lazy'] = opanda_get_option('lazy', false)
				? true
				: false;
			...
			$lockData['stats'] = get_option('opanda_tracking', false)
				? true
				: false;
			...
			if( $dynamicTheme ) { ?>
				...
				<?php
				//todo: хук является устаревшим opanda_print_locker_assets
				factory_000_do_action_deprecated("opanda_print_locker_assets", array(
					$this->lockData['lockerId'],
					$this->lockData,
					$this->lockId
				), '1.2.4', "bizpanda_print_locker_assets");

				do_action('bizpanda_print_locker_assets', $this->lockData['lockerId'], $this->lockData, $this->lockId);
				?>
			<?php } else {
