<?php
	...
	class OPanda_Items {
		...
		public static function getCurrentItemName()
		{
			...
			// - from the query
			$item = isset($_GET['opanda_item'])
				? $_GET['opanda_item']
				: null;

			// - from the form hidden field
			if( empty($item) ) {
				$item = isset($_POST['opanda_item'])
					? $_POST['opanda_item']
					: null;
			}
		...
		public static function getAvailable()
		{
			...
			//todo: Фильтр opanda_items устарел
			$items = factory_000_apply_filters_deprecated("opanda_items", array($items), '1.2.4', "bizpanda_items");
			self::$_available = apply_filters('bizpanda_items', $items);
		...
		public static function getPremiumUrl($name)
		{
			...
			if( isset($item['plugin']->options['premium']) ) {
				return $item['plugin']->options['premium'];
			}
