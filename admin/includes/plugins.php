<?php
	...
	class OPanda_Plugins {
		...
		public static function getAll()
		{
			...
			if( onp_lang('ru_RU') ) {
				$items[] = array(
					...
					'url' => 'https://sociallocker.ru/download/#optinpanda-purchase-anchor',
					'tags' => array('social', 'subscribers'),
					'pluginName' => 'optinpanda-rus'
				);
			...
			if( onp_lang('ru_RU') ) {
				$items[] = array(
					...
					'url' => 'https://sociallocker.ru/download/#sociallocker-purchase-anchor',
					'tags' => array('social', 'subscribers', 'sociallocker-ads'),
					'pluginName' => 'sociallocker-rus'
				);
			...
			//todo: Фильтр opanda_register_plugins устарел
			$items = factory_000_apply_filters_deprecated("opanda_register_plugins", array($items), '1.2.4', 'bizpanda_register_plugins');
			$items = apply_filters('bizpanda_register_plugins', $items);
		...
		public static function getSuggestions()
		{
			...
			//todo: Фильтр opanda_plugins_suggestions устарел
			$suggestions = factory_000_apply_filters_deprecated("opanda_plugins_suggestions", array($suggestions), '1.2.4', 'bizpanda_plugins_suggestions');
			$suggestions = apply_filters('bizpanda_plugins_suggestions', $suggestions);
			...
			if( false === strpos($url, 'utm_source') ) {
				...
				if( BizPanda::isSinglePlugin() ) {
					...
					$args = array(
						...
						'tracker' => isset($plugin->options['tracker'])
