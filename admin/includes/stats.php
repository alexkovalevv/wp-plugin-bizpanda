<?php
	...
	class OPanda_Stats {
		...
		public static function getChartData($options)
		{
			...
			$rangeStart = isset($options['rangeStart'])
				? $options['rangeStart']
				: null;
			$rangeEnd = isset($options['rangeEnd'])
				? $options['rangeEnd']
				: null;
			...
			$extraWhere = '';
			if( $postId ) {
				$extraWhere .= ' AND t.post_id=' . $postId;
			}
			if( $itemId ) {
				$extraWhere .= ' AND t.item_id=' . $itemId;
			}
		...
		public static function getViewTable($options)
		{
			...
			$count = ($total)
				? $wpdb->get_var("SELECT COUNT(Distinct t.post_id) FROM {$wpdb->prefix}opanda_stats_v2 AS t
            WHERE (aggregate_date BETWEEN '$rangeStartStr' AND '$rangeEndStr') $extraWhere")
				: 0;
			...
			foreach($rawData as $row) {
				...
				if( !isset($resultData[$postId]) ) {
					...
					if( empty($title) ) {
						$title = __('(the post not found)', 'bizpanda');
					}
		...
		public static function countMetrict($itemId, $postId, $metricName)
		{
			...
			if( empty($itemId) || empty($postId) ) {
				return;
			}

			$sql = $wpdb->prepare("INSERT INTO {$wpdb->prefix}opanda_stats_v2
            (aggregate_date,item_id,post_id,metric_name,metric_value) 
            VALUES (%s,%d,%d,%s,1)
            ON DUPLICATE KEY UPDATE metric_value = metric_value + 1", self::getCurrentMySqlDate(), $itemId, $postId, $metricName);
		...
		public static function processEvent($itemId, $postId, $eventName, $eventType)
		{
			...
			if( 'unlock' == $eventType ) {
				...
				if( get_option('opanda_notify_unlocks', false) ) {
					...
					//todo: хук является устаревшим opanda_unlocks_notification_subject
					$subject = factory_000_do_action_deprecated('opanda_unlocks_notification_subject', array($subject), '1.2.4', 'bizpanda_unlocks_notification_subject');
					$subject = apply_filters('bizpanda_unlocks_notification_subject', $subject);

					//todo: хук является устаревшим opanda_unlocks_notification_message
					$message = factory_000_do_action_deprecated('opanda_unlocks_notification_message', array($message), '1.2.4', 'bizpanda_unlocks_notification_message');
					$message = apply_filters('opanda_unlocks_notification_message', $message);
