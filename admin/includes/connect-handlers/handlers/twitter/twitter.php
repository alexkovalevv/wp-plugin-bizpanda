<?php
	...
	class OPanda_TwitterHandler extends OPanda_Handler {
		...
		public function handleRequest()
		{
			...
			$requestType = !empty($_REQUEST['opandaRequestType'])
				? $_REQUEST['opandaRequestType']
				: null;
			...
			$visitorId = !empty($_REQUEST['opandaVisitorId'])
				? $_REQUEST['opandaVisitorId']
				: null;

			$readOnly = !empty($_REQUEST['opandaReadOnly'])
				? (bool)$_REQUEST['opandaReadOnly']
				: null;
			if( $readOnly ) {
		...
		public function getCallbackUrl($visitorId, $keepOpen)
		{
			...
			return $proxy . $prefix . 'opandaHandler=twitter&opandaRequestType=callback&opandaVisitorId=' . $visitorId . ('&opandaKeepOpen=' . ($keepOpen
					? '1'
					: '0'));
		}
		...
		public function doInit($visitorId)
		{
			...
			$keepOpen = !empty($_REQUEST['opandaKeepOpen'])
				? (bool)$_REQUEST['opandaKeepOpen']
				: null;
			$readOnly = !empty($_REQUEST['opandaReadOnly'])
				? (bool)$_REQUEST['opandaReadOnly']
				: null;
		...
		protected function getTwitterOAuth($visitorId = null, $token = null, $secret = null)
		{
			...
			if( empty($token) ) {
				...
				if( empty($token) ) {
					throw new Opanda_HandlerException("The access token not found for $visitorId");
				}
			}
			if( empty($secret) ) {
				$secret = $this->getValue($visitorId, 'twitter_secret');
				if( empty($secret) ) {
					throw new Opanda_HandlerException("The secret of the access token is invalid for $visitorId");
				}
			}
		...
		protected function follow($visitorId)
		{
			...
			$followTo = isset($_REQUEST['opandaFollowTo'])
				? $_REQUEST['opandaFollowTo']
				: null;
			if( empty($followTo) ) {
				throw new Opanda_HandlerException("The user name to follow is not specified");
			}
			...
			//todo: хук является устаревшим opanda_got_twitter_follower
			factory_000_do_action_deprecated('opanda_got_twitter_follower', array($contextData), '1.2.4', 'bizpanda_got_twitter_follower');

			do_action('bizpanda_got_twitter_follower', $contextData);
		...
		protected function tweet($visitorId)
		{
			...
			$message = !empty($_REQUEST['opandaTweetMessage'])
				? $_REQUEST['opandaTweetMessage']
				: null;
			if( empty($message) ) {
				throw new Opanda_HandlerException("The tweet text is not specified.");
			}
			...
			//todo: хук является устаревшим opanda_tweet_posted
			factory_000_do_action_deprecated('opanda_tweet_posted', array($contextData), '1.2.4', 'bizpanda_tweet_posted');

			do_action('bizpanda_tweet_posted', $contextData);
