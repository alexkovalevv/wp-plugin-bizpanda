<?php
	...
	class OPanda_ItemsViewTable extends FactoryViewtables000_Viewtable {
		...
		public function configure()
		{
			...
			$this->styles->add(OPANDA_BIZPANDA_URL . '/assets/admin/css/item-view.css');
		...
		public function columnTitle($post, $isFullMode)
		{
			if( $isFullMode ) {
				...
				if( empty($url) ) {
					$url = '<i>[' . __('current page', 'bizpanda') . ']</i>';
				}
		...
		public function actionPostRowActions($actions)
		{
			...
			$temporaryActions = array(
				'edit' => $actions['edit'],
				'clone' => '<a href="' . wp_nonce_url(get_admin_url() . 'post.php?post_type=opanda-item&amp;post=' . $post->ID . '&amp;action=opanda-clone-item', 'opanda-clone-item-nonce') . '" title="' . __("Clone this element", 'bizpanda') . '">' . __("Clone", 'bizpanda') . '</a>',
				'trash' => $actions['trash']
			);
