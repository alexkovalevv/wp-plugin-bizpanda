<?php
	...
	abstract class OPanda_Subscription {
		...
		public function wpCheck($identity, $serviceReady, $context, $listId, $verified)
		{
			...
			$lead = OPanda_Leads::getByEmail($email);
			if( !$lead ) {
				throw new OPanda_SubscriptionException(__('The email is not found. Please try again.', 'optinpanda'));
			}

			return array(
				'status' => $lead->lead_subscription_confirmed
					? 'subscribed'
					: 'pending'
			);
		}
			...
			if( $this->isTransactional() ) {
				...
				if( !$result ) {
					if( $this->deliveryError ) {
						throw new OPanda_SubscriptionException($this->deliveryError->get_error_message());
