<?php
	...
	class OPanda_LeadsListTable extends WP_List_Table {
		...
		public function no_items()
		{
			...
			if( BizPanda::isSinglePlugin() ) {
				...
				if( BizPanda::hasPlugin('optinpanda') ) {
					printf(__('To start generating leads, create <a href="%s"><strong>Email Locker</strong></a> or <a href="%s"><strong>Sign-In Locker</strong></a> and lock with them some content on your website.', 'bizpanda'), opanda_get_help_url('what-is-email-locker'), opanda_get_help_url('what-is-signin-locker'));
				} else {
					printf(__('To start generating leads, create <a href="%s"><strong>Sign-In Locker</strong></a> and lock with it some content on your website.', 'bizpanda'), opanda_get_help_url('what-is-signin-locker'));
				}
			} else {
				printf(__('To start generating leads, create <a href="%s"><strong>Email Locker</strong></a> or <a href="%s"><strong>Sign-In Locker</strong></a> and lock with them some content on your website.', 'bizpanda'), opanda_get_help_url('what-is-email-locker'), opanda_get_help_url('what-is-signin-locker'));
			}
		...
		function prepare_items()
		{
			...
			$where = array();
			if( isset($_GET['opanda_status']) && in_array($_GET['opanda_status'], array(
					'confirmed',
					'not-confirmed'
				))
			) {
				$where[] = 'lead_email_confirmed = ' . (($_GET['opanda_status'] == 'confirmed')
						? '1'
						: '0');
			}
			...
			// order
			$allowed = apply_filters('bizpanda_leads_allowed_orderby_columns', array(
				'name',
				'channel',
				'added',
				'status'
			...
			//todo: Фильтр opanda_leads_allowed_orderby_columns устарел
			if( empty($allowed) ) {
				$allowed = factory_000_apply_filters_deprecated("opanda_leads_allowed_orderby_columns", array(
					array(
						'name',
						'channel',
						'added',
				...
				), '1.2.4', 'bizpanda_leads_allowed_orderby_columns');
			}
			...
			if( 'name' === $orderby ) {
				$dbOrderBy = array('lead_display_name', 'lead_email');
			} elseif( 'channel' === $orderby ) {
				$dbOrderBy = array('lead_item_id');
			} elseif( 'added' === $orderby ) {
				$dbOrderBy = array('lead_date');
			} elseif( 'status' === $orderby ) {
				$dbOrderBy = array('lead_confirmed');
			...
			$totalitems = $wpdb->query(str_replace("SELECT *", "SELECT COUNT(id)", $query));
			$perpage = 20;
		...
		{
			return sprintf('<input type="checkbox" name="opanda_leads[]" value="%s" />', $record->ID);
		}
		...
		public function column_name($record)
		{
			...
			if( !empty($url) ) {
				$name .= '<a href="' . $url . '" class="opanda-name">';
			...
			if( isset($fields['vkUrl']) ) {
				$name .= sprintf('<a href="%s" target="_blank" class="opanda-social-icon opanda-vk-icon"><i class="fa  fa-vk"></i></a>', $fields['vkUrl']);
			}
		...
		public function column_status($record)
		{
			...
			if( BizPanda::hasPlugin('optinpanda') ) {
				...
				if( $record->lead_email_confirmed ) { ?>
                    ...
                    <i class="fa fa-check-circle-o"></i><i><?php _e('email', 'bizpanda') ?></i>
                </span>
				<?php } else { ?>
                    ...
                    <i class="fa fa-circle-o"></i><i><?php _e('email', 'bizpanda') ?></i>
                </span>
				<?php }
				...
				if( $record->lead_subscription_confirmed ) { ?>
                    ...
                    <i class="fa fa-check-circle-o"></i><i><?php _e('subscription', 'bizpanda') ?></i>
                </span>
				<?php } else { ?>
					<span class="opanda-status-help" title="<?php _e('This user has not confirmed his subscription.', 'bizpanda') ?>">
                    <i class="fa fa-circle-o"></i><i><?php _e('subscription', 'bizpanda') ?></i>
                </span>
				<?php }
			} else {
		...
		public function column_default($item, $column_name)
		{
			...
			//todo: Фильтр opanda_leads_column устарел
			$value = factory_000_apply_filters_deprecated("opanda_leads_column", array(
				$value,
				$item,
				$column_name
			), '1.2.4', 'bizpanda_leads_column');

			$value = apply_filters('bizpanda_leads_column', $value, $item, $column_name);
