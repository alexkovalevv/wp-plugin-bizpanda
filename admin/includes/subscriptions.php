<?php
	...
	class OPanda_SubscriptionServices {
		...
		{
			$result = apply_filters('bizpanda_subscription_services', array());

			//todo: Фильтр opanda_subscription_services устарел
			if( empty($result) ) {
				$result = factory_000_apply_filters_deprecated("opanda_subscription_services", array(array()), '1.2.4', 'bizpanda_subscription_services');
			}
		...
		public static function getServiceInfo($name = null)
		{
			...
			$name = empty($name)
				? get_option('opanda_subscription_service', 'none')
				: $name;
		...
		public static function getAllOptinModes()
		{
			...
			//todo: Фильтр opanda_optin_modes устарел
			$modes = factory_000_apply_filters_deprecated("opanda_optin_modes", array($modes), '1.2.4', 'bizpanda_optin_modes');
			$modes = apply_filters('bizpanda_optin_modes', $modes);
