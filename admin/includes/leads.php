<?php
	...
	class OPanda_Leads {
		...
		public static function save($lead = null, $identity = array(), $context = array(), $emailConfirmed = false, $subscriptionConfirmed = false, $temp = null)
		{
			...
			if( !$leadId ) {
				...
				$data = array(
					...
					'lead_email_confirmed' => $emailConfirmed
						? 1
						: 0,
					'lead_subscription_confirmed' => $subscriptionConfirmed
						? 1
						: 0,
					'lead_ip' => self::getIP(),
					'lead_temp' => !empty($temp)
						? json_encode($temp)
						: null
				);
				...
				if( get_option('opanda_notify_leads', false) ) {
					...
					//todo: хук является устаревшим opanda_leads_notification_subject
					$subject = factory_000_do_action_deprecated('opanda_leads_notification_subject', array(
						$subject,
						$data
					), '1.2.4', 'bizpanda_leads_notification_subject');
					$subject = apply_filters('bizpanda_leads_notification_subject', $subject, $data);

					//todo: хук является устаревшим opanda_leads_notification_message
					$message = factory_000_do_action_deprecated('opanda_leads_notification_message', array(
						$message,
						$data
					), '1.2.4', 'bizpanda_leads_notification_message');
		...
		public static function confirm($email, $code, $push = false)
		{
			...
			if( !$lead || $lead->lead_subscription_confirmed ) {
				return;
			}
			...
			if( $code !== $lead->lead_confirmation_code ) {
				return;
			}

			$temp = !empty($lead->lead_temp)
				? json_decode($lead->lead_temp, true)
				: null;

			$itemId = isset($temp['context']['itemId'])
				? $temp['context']['itemId']
				: null;
			$postId = isset($temp['context']['postId'])
				? $temp['context']['postId']
				: null;
		...
		{
			if( isset(self::$_leads[$leadId]) ) {
				return self::$_leads[$leadId];
			}
		...
		public static function getSocialUrl($leadId)
		{
			...
			if( isset($fields['facebookUrl']) ) {
				return $fields['facebookUrl'];
			}
			if( isset($fields['twitterUrl']) ) {
				return $fields['twitterUrl'];
			}
			if( isset($fields['googleUrl']) ) {
				return $fields['googleUrl'];
			}
			if( isset($fields['linkedinUrl']) ) {
				return $fields['linkedinUrl'];
			}
		...
		public static function getCountByStatus()
		{
			...
			$rows = $wpdb->get_results("SELECT COUNT(*) as status_count, lead_email_confirmed FROM {$wpdb->prefix}opanda_leads GROUP BY lead_email_confirmed", ARRAY_A);
