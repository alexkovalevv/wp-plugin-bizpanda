<?php
	...
	function opanda_statistics()
	{
		...
		$statsItem = isset($_POST['opandaStats'])
			? $_POST['opandaStats']
			: array();
		$contextData = isset($_POST['opandaContext'])
			? $_POST['opandaContext']
			: array();

		// event name

		$eventName = isset($statsItem['eventName'])
			? $statsItem['eventName']
			: null;
		...
		$eventType = isset($statsItem['eventType'])
			? $statsItem['eventType']
			: null;
		...
		$visitorId = isset($statsItem['visitorId'])
			? $statsItem['visitorId']
			: null;
		...
		$context = isset($_POST['opandaContext'])
			? $_POST['opandaContext']
			: array();
		...
		if( $eventName == 'form' && $eventType == 'unlock' ) {
			...
			if( $unlocked ) {
				return json_encode(array('error' => __('Already counted.', 'bizpanda')));
			}
