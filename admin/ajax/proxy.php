<?php
	/**
	 * Фикс для кнопки вконтакте, так как в redirect_url нельзя передать параметры,
	 * мы отправляем пользователя на уникальный хук, чтобы идентифицировать Вк авторизацию
	 *
	 * @since 1.0.0
	 * @return void
	 */
	function opanda_vk_connect()
	{
		$_REQUEST['opandaHandler'] = 'vk';
		opanda_connect();
		exit;
	}

	add_action('wp_ajax_opanda_vk_connect', 'opanda_vk_connect');
	add_action('wp_ajax_nopriv_opanda_vk_connect', 'opanda_vk_connect');
	...
	function opanda_connect()
	{
		...
		$handlerName = isset($_REQUEST['opandaHandler'])
			? $_REQUEST['opandaHandler']
			: null;
		$allowed = array('vk', 'twitter', 'linkedin', 'subscription', 'signup', 'lead');
	...
	function opanda_get_custom_fields()
	{
		...
		try {
			...
			$listId = isset($_POST['opanda_list_id'])
				? $_POST['opanda_list_id']
				: null;
			$service = OPanda_SubscriptionServices::getCurrentService();
