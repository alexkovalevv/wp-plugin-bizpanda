<?php
	...
	class OPanda_StatisticsPage extends OPanda_AdminPage {
		...
		public function assets($scripts, $styles)
		{
			...
			$this->scripts->add(OPANDA_BIZPANDA_URL . '/assets/admin/js/stats.js');
			$this->styles->add(OPANDA_BIZPANDA_URL . '/assets/admin/css/stats.css');
		}
		...
		public function indexAction()
		{
			...
			$itemId = isset($_GET['opanda_id'])
				? $_GET['opanda_id']
				: null;
			if( empty($itemId) ) {
				$itemId = isset($dropdownItems[0]->ID)
					? $dropdownItems[0]->ID
					: 0;
			}
			...
			$screens = apply_filters("bizpanda_item_type_stats_screens", array(), $itemName);

			//todo: Фильтр opanda_item_type_stats_screens устарел
			if( empty($screens) ) {
				$screens = factory_000_apply_filters_deprecated("opanda_item_type_stats_screens", array(
					array(),
					$itemName
				), '1.2.4', "bizpanda_item_type_stats_screens");
			}

			//todo: Фильтр opanda_{$itemName}_stats_screens устарел
			$screens = factory_000_apply_filters_deprecated("opanda_{$itemName}_stats_screens", array($screens), '1.2.4', "bizpanda_{$itemName}_stats_screens");
			$screens = apply_filters("bizpanda_{$itemName}_stats_screens", $screens);
			...
			$currentScreenName = isset($_REQUEST['opanda_screen'])
				? $_REQUEST['opanda_screen']
				: 'summary';
			...
			$screenClass = isset($currentScreen['screenClsss'])
				? $currentScreen['screenClsss']
				: 'OPanda_StatsScreen';
			...
			$postId = isset($_REQUEST['opanda_post_id'])
				? intval($_REQUEST['opanda_post_id'])
				: false;
			$post = ($postId)
				? get_post($postId)
				: false;
			...
			$dateStart = isset($_REQUEST['opanda_date_start'])
				? $_REQUEST['opanda_date_start']
				: false;
			$dateEnd = isset($_REQUEST['opanda_date_end'])
				? $_REQUEST['opanda_date_end']
				: false;
			...
			$page = (isset($_GET['opanda_page']))
				? intval($_GET['opanda_page'])
				: 1;
			if( $page <= 0 ) {
				$page = 1;
			}
			...
			<div class="wrap">
				...
				<div id="opanda-control-panel">
					<div class="opanda-left" id="opanda-current-item">
						<span><?php _e('You are viewing reports for ', 'bizpanda') ?>
							<a href="<?php echo admin_url("post.php?post=" . $itemId . "&action=edit") ?>"><strong><?php echo $itemTitle ?></strong></a></span>
					</div>
					<form method="get" id="opanda-item-selector" class="opanda-right">
						...
						<span><?php _e('Select item to view:', 'bizpanda') ?></span>
						<select name="opanda_id">
							<?php foreach($dropdownItems as $dropdownItem) { ?>
								<option value="<?php echo $dropdownItem->ID ?>" <?php if( $dropdownItem->ID == $itemId ) {
								...
								<div id="onp-sl-type-select">
									<div class="btn-group" id="chart-type-group" data-toggle="buttons-radio">
										<?php foreach($screens as $screenName => $screen) { ?>
											<a href="<?php echo add_query_arg('opanda_screen', $screenName, $urlBase) ?>" class="btn btn-default <?php if( $screenName == $currentScreenName ) {
												echo 'active';
											} ?> type-<?php echo $screenName ?>" data-value="<?php echo $screenName ?>"><?php echo $screen['title'] ?></a>
										<?php } ?>
									</div>
								</div>
							...
							<?php if( $table->hasComplexColumns() ) { ?>
								...
								<tr>
									<?php foreach($table->getHeaderColumns() as $name => $column) { ?>
										<th rowspan="<?php echo $column['rowspan'] ?>" colspan="<?php echo $column['colspan'] ?>" class="opanda-col-<?php echo $name ?> <?php echo isset($column['cssClass'])
											? $column['cssClass']
			...
			<div id="opanda-locker-select-popup" style="display: none;">
				...
				<select id="opanda-locker-select">
					<?php foreach($dropdownItems as $dropdownItem) { ?>
						<option value="<?php echo opanda_get_admin_url('stats', array('opanda_id' => $dropdownItem->ID)); ?>" <?php if( $dropdownItem->ID == $itemId ) {
	...
	FactoryPages000::register($bizpanda, 'OPanda_StatisticsPage');