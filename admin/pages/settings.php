<?php
	 ...
	 * @since 1.0.0
	 */
	class OPanda_SettingsPage extends FactoryPages000_AdminPage {
		...
		public function assets($scripts, $styles)
		{
			...
			$this->scripts->add(OPANDA_BIZPANDA_URL . '/assets/admin/js/settings.js');
			$this->styles->add(OPANDA_BIZPANDA_URL . '/assets/admin/css/settings.css');
			/*@mix:place*/
		}
		...
		public function indexAction()
		{
			...
			$current = isset($_GET['opanda_screen'])
				? $_GET['opanda_screen']
				: null;
			$screens = array();
			...
			//todo: Фильтр opanda_settings_screens устарел
			$screens = factory_000_apply_filters_deprecated("opanda_settings_screens", array($screens), '1.2.4', 'bizpanda_settings_screens');
			$screens = apply_filters('bizpanda_settings_screens', $screens);
			...
			$action = isset($_GET['opanda_action'])
				? $_GET['opanda_action']
				: null;
			if( !empty($action) ) {
			...
			//todo: Фильтр opanda_{$current}_settings устарел
			$options = factory_000_apply_filters_deprecated("opanda_{$current}_settings", array($options), '1.2.4', "bizpanda_{$current}_settings");
			$options = apply_filters("bizpanda_{$current}_settings", $options);
			...
			if( isset($_POST['save-action']) ) {
				//todo: хук является устаревшим opanda_{$current}_settings_saving
				factory_000_do_action_deprecated("opanda_{$current}_settings_saving", array(), '1.2.4', "bizpanda_{$current}_settings_saving");
				
				do_action("bizpanda_{$current}_settings_saving");
				$form->save();
				
				//todo: хук является устаревшим opanda_{$current}_settings_saved
				factory_000_do_action_deprecated("opanda_{$current}_settings_saved", array(), '1.2.4', "bizpanda_{$current}_settings_saved");
				
				do_action("bizpanda_{$current}_settings_saved");
				
				$redirectArgs = apply_filters("bizpanda_{$current}_settings_redirect_args", array(
					'opanda_saved' => 1,
					'opanda_screen' => $current
				));
				
				//todo: Фильтр opanda_{$current}_settings_redirect_args устарел
				if( empty($redirectArgs) ) {
					$redirectArgs = factory_000_apply_filters_deprecated("opanda_{$current}_settings_redirect_args", array(
						array(
							'opanda_saved' => 1,
							'opanda_screen' => $current
						)
					), '1.2.4', "bizpanda_{$current}_settings_redirect_args");
				}
            ...
            <div class="wrap ">
                <h2 class="nav-tab-wrapper">
					<?php foreach($screens as $screenName => $screenData) { ?>
                    <a href="<?php $this->actionUrl('index', array('opanda_screen' => $screenName)) ?>" class="nav-tab <?php if( $screenName === $current ) {
						echo 'nav-tab-active';
					} ?>">
				...
				<?php $screen->header() ?>
                <div class="factory-bootstrap-000 factory-fontawesome-000 opanda-screen-<?php echo $current ?>">
                    <form method="post" class="form-horizontal" action="<?php echo $formAction ?>">
						...
						<?php
							//todo: хук является устаревшим onp_sl_settings_options_notices
							factory_000_do_action_deprecated("onp_sl_settings_options_notices", array(), '1.2.4', "bizpanda_settings_options_notices");
							
							do_action('bizpanda_settings_options_notices');
						?>
	...
	FactoryPages000::register($bizpanda, 'OPanda_SettingsPage');
