<?php
	...
	class OPanda_LeadsPage extends OPanda_AdminPage {
		...
		{
			$this->styles->add(OPANDA_BIZPANDA_URL . '/assets/admin/css/leads.css');
			$this->scripts->add(OPANDA_BIZPANDA_URL . '/assets/admin/js/leads.js');
		...
		public function indexAction()
		{
			...
			?>
			<div class="wrap factory-fontawesome-000" id="opanda-leads-page">
				<h2>
				...
				<?php
					$table->search_box(__('Search Leads', 'bizpanda'), 's');
					$table->views();
				?>
		...
		{
			$leadId = isset($_REQUEST['leadID'])
				? intval($_REQUEST['leadID'])
				: 0;
			...
			if( !empty($name) || !empty($family) ) {
				...
				$displayName = !empty($lead->lead_display_name)
					? $lead->lead_display_name
					: $lead->lead_email;
			}

			$emailConfirmed = empty($lead->lead_email_confirmed)
				? 0
				: 1;
			$subscriptionConfirmed = empty($lead->lead_subscription_confirmed)
				? 0
				: 1;
			...
			if( isset($_POST['submit']) ) {
				...
				if( !filter_var($email, FILTER_VALIDATE_EMAIL) || !preg_match('/@.+\./', $email) ) {
					...
					$emailConfirmed = empty($_POST['email_confirmed'])
						? 0
						: 1;
					$subscriptionConfirmed = empty($_POST['subscription_confirmed'])
						? 0
						: 1;

					$customValues = isset($_POST['opanda_values'])
						? $_POST['opanda_values']
						: array();
					$customNames = isset($_POST['opanda_names'])
						? $_POST['opanda_names']
						: array();
			...
			<div class="wrap factory-fontawesome-000" id="opanda-lead-details-page">
							...
							<td class="user-info">
								<h3 class="detail">
									<ul class="click-to-edit">
										...
										<li>
											<input id="opanda_email" class="" type="text" name="email" value="<?php echo htmlspecialchars($email) ?>" placeholder="<?php _e('Email', 'bizpanda') ?>">
										</li>
									</ul>
								</h3>
								<div class="detail">
								...
								<div class="detail">
									...
									<ul class="click-to-edit">
										<li><?php if( $emailConfirmed ) {
												_e('yes', 'bizpanda');
											} else {
												_e('no', 'bizpanda');
											} ?></li>
										<li>
											<input type="checkbox" id="opanda_email_confirmed" name="email_confirmed" value="1" <?php if( $emailConfirmed ) {
								...
								<?php if( BizPanda::hasPlugin('optinpanda') ) { ?>
									...
									<div class="detail">
										...
										<ul class="click-to-edit">
											<li><?php if( $subscriptionConfirmed ) {
													_e('yes', 'bizpanda');
												} else {
													_e('no', 'bizpanda');
												} ?></li>
											<li>
												<input type="checkbox" id="opanda_email_confirmed" name="subscription_confirmed" value="1" <?php if( $subscriptionConfirmed ) {
													echo 'checked="checked"';
												} ?> >
											</li>
										</ul>
									...
									<div class="custom-field-wrap">
										<?php
											...
											foreach($customFields as $fieldName => $fieldValue) {
												...
												<div class="detail">
													<label for="opanda_<?php echo $index ?>"><?php echo $fieldName ?>
														:</label>
													<ul class="click-to-edit">
														<li><?php echo $fieldValue ?></li>
														<li>
															<input type="text" id="opanda_<?php echo $index ?>" name="opanda_values[]" value="<?php echo $fieldValue ?>" class="regular-text input">
														</li>
		...
		public function exportAction()
		{
			...
			if( !onp_build('free') ) {
				...
				if( isset($_POST['opanda_export']) ) {
					...
					$delimiter = isset($_POST['opanda_delimiter'])
						? $_POST['opanda_delimiter']
						: ',';
					...
					$lockers = isset($_POST['opanda_lockers'])
						? $_POST['opanda_lockers']
						: array();
					$lockerIds = array();
					...
					$status = isset($_POST['opanda_status'])
						? $_POST['opanda_status']
						: 'all';
					...
					$rawFields = isset($_POST['opanda_fields'])
						? $_POST['opanda_fields']
						: array();
					$fields = array();
					...
					$rawCustomFields = isset($_POST['opanda_custom_fields'])
						? $_POST['opanda_custom_fields']
						: array();
					$selectedCustomFields = array();
					...
					if( empty($lockers) || empty($fields) ) {
						...
						if( 'all' != $status ) {
							$sql .= ' AND leads.lead_email_confirmed = ' . (('confirmed' == $status)
									? '1'
									: '0');
						}
		...
		{
			?>
			<div class="onp-page-wrap factory-bootstrap-000" id="onp-confirm-dialog">
	...
	FactoryPages000::register($bizpanda, 'OPanda_LeadsPage');
/*@mix:place*/