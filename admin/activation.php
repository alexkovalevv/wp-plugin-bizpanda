<?php
	 ...
	 * @since 1.0.0
	 */
	class OPanda_Activation extends Factory000_Activator {
		...
		public function activate()
		{
			...
			//todo: хук является устаревшим before_bizpanda_activation
			factory_000_do_action_deprecated('before_bizpanda_activation', array(
				$bizpanda,
				$this
			), '1.2.4', 'bizpanda_before_activation');
			
			do_action('bizpanda_before_activation', $bizpanda, $this);
			...
			//todo: хук является устаревшим after_bizpanda_activation
			factory_000_do_action_deprecated('after_bizpanda_activation', array(
				$bizpanda,
				$this
			), '1.2.4', 'bizpanda_after_activation');
			
			do_action('bizpanda_after_activation', $bizpanda, $this);
		}
		...
		protected function importOptions()
		{
			...
			$wpdb->query("UPDATE IGNORE {$wpdb->options} SET option_name = REPLACE(option_name, 'optinpanda_', 'opanda_') WHERE option_name LIKE 'optinpanda_%'");
			$wpdb->query("UPDATE IGNORE {$wpdb->postmeta} SET meta_key = REPLACE(meta_key, 'optinpanda_', 'opanda_') WHERE meta_key LIKE 'optinpanda_%'");
			
			// Convers options of the Social Locker to the options of the Opt-In Panda
			
			$wpdb->query("UPDATE IGNORE {$wpdb->options} SET option_name = REPLACE(option_name, 'sociallocker_', 'opanda_') WHERE option_name LIKE 'sociallocker_%'");
			$wpdb->query("UPDATE IGNORE {$wpdb->postmeta} SET meta_key = REPLACE(meta_key, 'sociallocker_', 'opanda_') WHERE meta_key LIKE 'sociallocker_%'");
		}
		...
		protected function presetOptions()
		{
			...
			add_option('opanda_facebook_version', 'v2.8');
			...
			update_option('opanda_lang', get_locale());
			
			$locale = get_locale();
			update_option('opanda_short_lang', substr($locale, 0, strpos($locale, '_')));
		}
		...
		protected function createPolicies()
		{
			...
			update_option('opanda_terms_enabled', 1);
			update_option('opanda_terms_use_pages', 0);
			
			if( get_locale() == 'ru_RU' ) {
				update_option('opanda_terms_of_use_text', file_get_contents(OPANDA_BIZPANDA_DIR . '/content/terms-of-use-ru_RU.html'));
				update_option('opanda_privacy_policy_text', file_get_contents(OPANDA_BIZPANDA_DIR . '/content/privacy-policy-ru_RU.html'));
			} else {
				update_option('opanda_terms_of_use_text', file_get_contents(OPANDA_BIZPANDA_DIR . '/content/terms-of-use.html'));
				update_option('opanda_privacy_policy_text', file_get_contents(OPANDA_BIZPANDA_DIR . '/content/privacy-policy.html'));
			}
		}
