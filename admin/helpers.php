<?php
	...
	function opanda_get_premium_url($name = null, $campaign = 'na')
	{
		...
		//todo: Фильтр opanda_premium_url устарел
		$url = factory_000_apply_filters_deprecated("opanda_premium_url", array(
			$url,
			$name,
			$campaign
		), '1.2.4', 'bizpanda_premium_url');
		$url = apply_filters('bizpanda_premium_url', $url, $name, $campaign);
	...
	function opanda_get_premium_note($wrap = true, $campaign = 'na')
	{
		...
		if( $wrap ) {
			$content .= '<div class="factory-fontawesome-000 opanda-overlay-note opanda-premium-note">';
		}
	...
	function opanda_print_simple_visibility_options($postId)
	{
		...
		<ul>
			...
			<?php } ?>
			<?php if( $relock ) { ?>
				<li><?php printf(__('ReLock: <strong>%s</strong>', 'bizpanda'), ($relock
						? ($relockInterval . ' ' . $relockIntervalUnits)
						: 'no')) ?></li>
			<?php } ?>
			<?php if( !$mobile ) { ?>
			...
			<?php } ?>
			<?php if( has_action('opanda_print_simple_visibility_options') ): ?>
				<?php
				//todo: хук является устаревшим opanda_print_simple_visibility_options
				factory_000_do_action_deprecated('opanda_print_simple_visibility_options', array(
					$postId,
					$empty
				), '1.2.4', 'bizpanda_print_simple_visibility_options');
				?>
				<?php do_action('bizpanda_print_simple_visibility_options', $postId, $empty); ?>
			<?php else: ?>
				<?php if( $empty ) { ?>
					<li>—</li>
				<?php } ?>
