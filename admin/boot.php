<?php
	...
	//require_once OPANDA_BIZPANDA_DIR . '/admin/pages/addons.php';
	#endcomp
	...
	if( isset($_REQUEST['action']) ) {
		switch( $_REQUEST['action'] ) {
			case 'opanda_search_post':
				require OPANDA_BIZPANDA_DIR . '/admin/ajax/search.php';
				break;
			case 'opanda_search_cats':
				require OPANDA_BIZPANDA_DIR . '/admin/ajax/search.php';
				break;
			case 'onp_sl_preview':
				...
				foreach($submenu['edit.php?post_type=' . OPANDA_POST_TYPE] as $menuKey => $menuValue) {
					if( !BizPanda::isSinglePlugin() ) {
						...
						foreach($plugins as $plugin) {
							$licenseManager = 'license-manager-' . $plugin['plugin']->pluginName;
							if( $menuValue[2] === $licenseManager ) {
								$submenu['edit.php?post_type=' . OPANDA_POST_TYPE][$menuKey][0] = $menuValue[0] . "<br>(" . $plugin['plugin']->pluginTitle . ")";
								$submenu['edit.php?post_type=' . OPANDA_POST_TYPE][$menuKey][3] = $menuValue[3] . "<br>(" . $plugin['plugin']->pluginTitle . ")";
							}
						}
					}
				}
	...
	function opanda_redirect_to_new_item()
	{
		...
		if( 'add' !== $screen->action || 'post' !== $screen->base || OPANDA_POST_TYPE !== $screen->post_type ) {
			return;
		}
		if( isset($_GET['opanda_item']) ) {
			return;
		}
	...
	function opanda_add_plugin($plugin_array)
	{
		...
		} else if( version_compare($wp_version, '4.3', '<') ) {
			$plugin_array['optinpanda'] = OPANDA_BIZPANDA_URL . '/assets/admin/js/optinpanda.tinymce4.js';
		} else {
			$plugin_array['optinpanda'] = OPANDA_BIZPANDA_URL . '/assets/admin/js/optinpanda.tinymce4.4.js';
		}
	...
	function opanda_subscribe($status, $identity, $context, $isWpSubscription)
	{
		...
		if( 'subscribed' == $status ) {
			...
			$confirmed = $serviceName === 'database'
				? false
				: true;
	...
	function opanda_registered($identity, $context = array())
	{
		...
		$itemId = isset($context['itemId'])
			? intval($context['itemId'])
			: 0;
		$postId = isset($context['postId'])
			? intval($context['postId'])
			: null;
	...
	function opanda_clone_item()
	{
		...
		if( !isset($_GET['action']) || $_GET['action'] !== 'opanda-clone-item' ) {
			return;
		}
		if( !isset($_GET['post_type']) || $_GET['post_type'] !== 'opanda-item' ) {
			return;
		}
		...
		if( !wp_verify_nonce($_GET['_wpnonce'], 'opanda-clone-item-nonce') ) {
			return;
		}
	...
	function opanda_add_meta_boxes()
	{
		...
		//todo: Фильтр opanda_item_type_metaboxes устарел
		$data = factory_000_apply_filters_deprecated("opanda_item_type_metaboxes", array(
			$data,
			$typeName
		), '1.2.4', 'bizpanda_item_type_metaboxes');
		$data = apply_filters("bizpanda_item_type_metaboxes", $data, $typeName);

		//todo: Фильтр opanda_{$typeName}_type_metaboxes устарел
		$data = factory_000_apply_filters_deprecated("opanda_{$typeName}_type_metaboxes", array($data), '1.2.4', "bizpanda_{$typeName}_type_metaboxes");
		$data = apply_filters("bizpanda_{$typeName}_type_metaboxes", $data);

		foreach($data as $metabox) {
			require_once $metabox['path'];
			FactoryMetaboxes000::registerFor(new $metabox['class']($bizpanda), OPANDA_POST_TYPE, $bizpanda);
		}
	}
