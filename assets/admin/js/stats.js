...
(function($) {
	...
	window.bizpanda.statistics = {
		...
		initButtonSelector: function() {
			...
			$('th[class^="opanda-col-unlock-via-"], td[class^="opanda-col-unlock-via-"]')
				.hide();
			...
			for( var index in activeSelectors ) {
				...
				$(".opanda-col-" + activeSelectors[index])
					.show();
			}
		...
		toggleSelector: function(selectorName) {
			...
			if( $item.hasClass('opanda-active') ) {
				this.deactivateSelector(selectorName);
			} else {
				this.activateSelector(selectorName);
			}

			this.savePreSelectedButtons();
		},
		...
		savePreSelectedButtons: function() {
			...
			var buttons = {};
			$(".onp-sl-selector-item.opanda-active").each(function(i) {
				var buttonName = $(this).data("selector");
				buttons[i] = buttonName;
			});

			var activeTab = this.getActiveTab(),
				lockerType = $('#chart-type-group').find('.btn-default').length == 3 ? 'sociallocker' : 'other',
				activeTab = lockerType == 'sociallocker' && activeTab == 'channels' ? 'channels-2' : activeTab;

			localStorage.setItem('opanda-chart-' + activeTab + '-selectors', JSON.stringify(buttons));
		},

		getActiveTab: function() {
			return $('#chart-type-group').find('.btn-default.active').data('value');
		},
		...
		getDefaultSelectors: function() {
			if( !window.localStorage ) {
				return window.opanda_default_selectors;
			}
			var activeTab = this.getActiveTab(),
				lockerType = $('#chart-type-group').find('.btn-default').length == 3 ? 'sociallocker' : 'other',
				activeTab = lockerType == 'sociallocker' && activeTab == 'channels' ? 'channels-2' : activeTab,
				selectors = localStorage.getItem('opanda-chart-' + activeTab + '-selectors');

			if( !selectors ) {
				if( lockerType == 'sociallocker' ) {
					return ["unlock-via-facebook-like", "unlock-via-twitter-tweet", "unlock-via-google-plus"];
				}
				return [];
			}
		...
		initTypeSelector: function() {
			...
			$("#onp-sl-type-select button").click(function() {
				...
				if( window.localStorage ) {
					window.localStorage.setItem('admin-optinpanda-chart-type', type);
				}
		...
		drawChart: function(params) {
			...
			for( var column in row ) {
				...
				var color = 'PRIMARY_DARK' === row[column]['color']
					? window.factory.factoryBootstrap000.colors.primaryDark
					: row[column]['color'];
